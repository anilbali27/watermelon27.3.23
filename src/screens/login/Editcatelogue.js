...
export default function Editcatelogue({ route, navigation }) {
  ...
  const getParticularProduct = async () => {
    ...
    setCategorykey(result.data[0].category_name);
    setsubCategorykey(result.data[0].subcategory_name);
    setstatuskey(result.data[0].status_name);
    ...
    console.log(result.data[0].shelf_life, "base_uom");
  ...
  const selectOneFile = async () => {
    ...
    try {
      ...
      if (res.type && res.type === "cancel") {
        ...
        const base64String: string = await FileSystem.readAsStringAsync(
          res.uri,
          {
          ...
          {isstepthree && (
            <View style={[styles.width100]}>
              ...
              <View style={[styles.mb28]}>
                <Text style={[styles.font12, styles.textBlack, styles.marBtm4]}>
                  Catalogue for*
                </Text>
